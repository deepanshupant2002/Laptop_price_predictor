...
processor = st.number_input("Processor(in GHz)")
...
if st.button('Predict Price'):
    ...
    query = np.array([company,types,ram,weight,touchscreen,ips,ppi,processor,cpu,hdd,ssd,gpu,os])

    query = query.reshape(1,13)
    st.title("The predicted price of the laptop is " + str(round(np.exp(pipe.predict(query)[0]),2)))